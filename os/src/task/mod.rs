...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        if task0.task_start_time.is_none() {task0.task_start_time = Some(get_time());}
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].task_start_time.is_none() {inner.tasks[next].task_start_time = Some(get_time());}
